...
import cn.merryyou.converter.OrderMaster2OrderDTOConverter;
...
//            cartDTOList.add(cartDTO);
        ...
        orderMaster.setOrderStatus(OrderStatusEnus.NEW.getCode());
    ...
    public OrderDTO cancle(OrderDTO orderDTO) {
        OrderMaster orderMaster = new OrderMaster();
        //判断订单的状态
        if (!orderDTO.getOrderStatus().equals(OrderStatusEnus.NEW.getCode())) {
            log.error("【取消订单】订单状态不正确，orderId={},orderStauts={}", orderDTO.getOrderId(), orderDTO.getOrderStatus());
            throw new SellException(ResultEnum.ORDER_STATUS_ERROR);
        }
        //修改订单状态、
        orderDTO.setOrderStatus(OrderStatusEnus.CANCLE.getCode());
        BeanUtils.copyProperties(orderDTO, orderMaster);
    ...
    public OrderDTO finish(OrderDTO orderDTO) {
        ...
        //修改状态
        orderDTO.setOrderStatus(OrderStatusEnus.FINISHED.getCode());
        OrderMaster orderMaster = new OrderMaster();
        BeanUtils.copyProperties(orderDTO, orderMaster);
        OrderMaster updateResult = orderMasterRepository.save(orderMaster);
        if (updateResult == null) {
    ...
    public OrderDTO paid(OrderDTO orderDTO) {
        //判断订单状态
        if (!orderDTO.getOrderStatus().equals(OrderStatusEnus.NEW.getCode())) {
            log.error("【订单支付】订单状态不正确，orderId={},orderStauts={}", orderDTO.getOrderId(), orderDTO.getOrderStatus());
            throw new SellException(ResultEnum.ORDER_STATUS_ERROR);
        }

        //判断支付状态
        if (!orderDTO.getPayStatus().equals(PayStatusEnum.WAIT.getCode())) {
            log.error("【订单支付】支付状态不正确，orderId={},payStatus={}", orderDTO.getOrderId(), orderDTO.getOrderStatus());
            throw new SellException(ResultEnum.ORDER_STATUS_ERROR);
        }
        //修改支付状态
